...
class InternationaSluglMixinFunctionnalRedirectionTest(InternationaSluglMixinTestMixin):
    ...
    def test_instance_creation_init_slug_with(self, model_name):
        instance = self.models.get(model_name).objects.create(
            field_fr="Field FR", field_en="Field en"
        )
    ...
    @data("slug_unicity_no_redirections", "no_slug_unicity_no_redirections")
    ...
    @data("slug_unicity_and_redirections", "no_slug_unicity_and_redirections")
    def test_instance_modification_creates_redirection(self, model_name):
        ...
        self.assertEqual(
            Redirect.objects.get(old_path="/fr/hello/").new_path, "/fr/world-fr/"
        )
    ...
    @data("slug_unicity_and_redirections", "no_slug_unicity_and_redirections")
    def test_instance_no_modification_doesnt_creates_redirection(self, model_name):
        instance = self.models.get(model_name).objects.create(field="hello")
        instance.save()
        self.assertEqual(Redirect.objects.all().count(), 0)

    @data("slug_unicity_and_redirections", "no_slug_unicity_and_redirections")
    def test_instance_multiple_modifications_create_multiple_redirections(
        self, model_name
    ):
        instance = self.models.get(model_name).objects.create(field="hello")
    ...
    def test_redirection_new_path_is_overriden(self, model_name):
        ...
        self.assertEqual(
            Redirect.objects.get(old_path="/fr/instance-fr/").new_path,
            "/fr/redirection-2-fr/",
        )
