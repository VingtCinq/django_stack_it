...
from stack_it.utils.templatetags.node_mixin import ContentNodeMixin
...
def pagetext(parser, token):
    ...
    if not (content_type[0] == content_type[-1] and content_type[0] in ('"', "'")):
        raise template.TemplateSyntaxError(f"content_type tag's argument should be in quotes: {content_type}")
    else:
        content_type = content_type.strip('"').strip("'")

    if not (key[0] == key[-1] and key[0] in ('"', "'")):
        raise template.TemplateSyntaxError(f"key tag's argument should be in quotes: {key}")
    else:
        key = key.strip('"').strip("'")

    if not (widget[0] == widget[-1] and widget[0] in ('"', "'")):
        raise template.TemplateSyntaxError(f"widget tag's argument should be in quotes: {widget}")
    else:
