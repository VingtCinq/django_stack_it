...
class ContentNodeMixin(template.Node):
    ...
    def __init__(self, instance, content_type, key, widget, nodelist):
        ...
        self.alternative_content_types = list(dict(BaseContentMixin.CONTENT_TYPES).keys())
    ...
    def create_content(self, instance, content_type, key, value):
        ...
        """
        attrs = dict([(self.INSTANCE_PARAMETER_NAME, instance),
                      ('content_type', content_type),
                      ('key', key),
                      ('value', value)])
        content_instance = self.CONTENT_MODEL.objects.create(**attrs)
        getattr(instance, f'{self.content_type}s').update(dict(((self.key, content_instance),)))
    ...
    def content(self, context):
        ...
        for content_type in self.alternative_content_types:
            ...
            if self.key in getattr(instance, f'{content_type}s').keys():
                ...
                msg = ('warning', f'Automatically changed {self.key} for instance {content_instance}!')
                self.messages.append(msg)
                getattr(instance, f'{content_type}s').pop(self.key)
                getattr(instance, f'{self.content_type}s').update({self.key: content_instance})
                return content_instance

        return self.create_content(instance, self.content_type, self.key, original_output)
